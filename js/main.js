...
class Layout{
    ...
    logoAnimation(){
        setInterval(()=>{
            ...
            if(this.welcomeTimer == 6){
                $('header .logo-wrapper').html('<h1 class="animate__animated animate__fadeIn">Welcome to 龍門!</h1>')
            }if(this.welcomeTimer == 0){
                this.welcomeTimer = 10
                $('header .logo-wrapper').html('<img src="img/elements/hilton_logo.png" class="animate__animated animate__fadeIn" alt="Hilton Casino Logo">')
            }
        },1000)
    }
    ...
    createGrid(){
        ...
        let mainRoadColSmall = 2 * Math.round( $(`${smallBoxElOne}`).outerWidth(true) / (this.tableColNumber/2) /2)
        ...
        let beadRoadItemSize = Math.round($(`${smallBoxEl} li`).outerHeight(true) - 2)
        let mainRoadItemSize = Math.round($(`${smallBoxElOne} li`).outerHeight(true) - 1)
        let bigeyeRoadItemSize = Math.round($(`${smallBoxElTwo} li`).outerHeight(true))
        let smallRoadItemSize = Math.round($(`${smallBoxElThree} li`).outerHeight(true) - 1)
        let cockRoadItemSize = Math.round($(`${smallBoxElFour} li`).outerHeight(true) - 1)
    ...
    toggleFullScreen(_this){
        if(!this.fsBool){
            ...
            if(window.innerWidth < window.innerHeight){
                ...
                $(_this).html('<i class="fa-solid fa-compress icon-dark"></i></a>')
            }
        }else{
            this.fsBool = false
            ...
            if(window.innerWidth < window.innerHeight){
                ...
                $(_this).html('<i class="fa-solid fa-expand icon-dark"></i></a>')
            }
        }
    }
    modalClose(){
